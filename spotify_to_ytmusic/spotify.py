...
class Spotify:
    ...
    def getAllSavedPlaylists(self):
        """
        Get all playlists saved by the current user, including:
        - Own playlists (public and private)
        - Followed playlists from other creators
        ...
        return [p for p in playlists if p["tracks"]["total"] > 0]

    def getSingleTrack(self, song_url):
        return self.api.track(song_url)

    def getSavedAlbums(self):
        """Get all saved albums from the current user's library"""
        albums = []
        response = self.api.current_user_saved_albums(limit=50)
        albums.extend(response["items"])

        while response["next"] is not None:
            response = self.api.current_user_saved_albums(
                limit=50, offset=response["offset"] + 50
            )
            albums.extend(response["items"])

        # Convert albums to playlist format
        album_playlists = []
        for album_item in albums:
            album = album_item["album"]
            # Get full album tracks
            tracks_response = self.api.album_tracks(album["id"])
            tracks = tracks_response["items"]
            
            # Handle pagination for albums with many tracks
            while tracks_response["next"] is not None:
                tracks_response = self.api.album_tracks(
                    album["id"], offset=tracks_response["offset"] + 50
                )
                tracks.extend(tracks_response["items"])
            
            album_playlists.append({
                "name": f"{album['name']} - {album['artists'][0]['name']}",
                "description": f"Album by {album['artists'][0]['name']} • {album['release_date']}",
                "tracks": build_results(tracks, album["name"]),
                "owner": {"display_name": album['artists'][0]['name']},
                "external_urls": {"spotify": album["external_urls"]["spotify"]},
                "type": "album"
            })
        
        return album_playlists
